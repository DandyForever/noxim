...
void Initiator::thread_process() {
  ...
  while (1) {
    ...
    // //////////////////////////////////////////////////////////////// if
    // explicitly set in the header flit, trasmission target should reach a
    // relay hub if (flit_payload.flit_type == FLIT_TYPE_HEAD)
    if (flit_payload.flit_type(FLIT_TYPE_HEAD)) {
      if (flit_payload.hub_relay_node != NOT_VALID) {
        ...
        LOG << "HUB RELAY: Flit " << flit_payload
            << " setting transmission hub relay " << current_hub_relay
            << " to reach destination " << endl;
      } else
    ...
    if (!trans->is_response_error()) {
      ...
      if (flit_payload.flit_type(FLIT_TYPE_TAIL)) {
        LOG << "*** [Ch" << _channel_id << "] tail flit sent " << flit_payload
            << ", releasing token" << endl;
