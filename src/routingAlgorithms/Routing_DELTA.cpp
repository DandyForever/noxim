...
RoutingAlgorithmsRegister
    Routing_DELTA::routingAlgorithmsRegister("DELTA", getInstance());
...
vector<int> Routing_DELTA::route(Router *router, const RouteData &routeData) {
  ...
  if (routeData.current_id < GlobalParams::n_delta_tiles)
  ...
  else {                     // for switch bloc
    int destination = routeData.dst_id;
    // LOG << "I am switch: " <<routeData.current_id << "  _Going to
    ...
    // LOG << "I am again switch: " <<routeData.current_id << "  _Going to
    // destination: " <<destination<< "  _Via direction "<<direction <<endl;
