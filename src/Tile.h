...
SC_MODULE(Tile)
{
    ...
    sc_in <bool> req_ry[DIRECTIONS];	        // The requests associated with the input channels
    sc_out <bool> ack_ry[DIRECTIONS];	        // The outgoing ack signals associated with the input channels
    sc_out <TBufferFullStatus> buffer_full_status_ry[DIRECTIONS];

    sc_out <Flit> flit_ty[DIRECTIONS];	// The output channels
    sc_out <bool> req_ty[DIRECTIONS];	        // The requests associated with the output channels
    sc_in <bool> ack_ty[DIRECTIONS];	        // The outgoing ack signals associated with the output channels
    ...
    sc_out <bool> hub_ack_ry;	        // The outgoing ack signals associated with the input channels
    ...
    sc_in <bool> hub_ack_ty;	        // The outgoing ack signals associated with the output channels
    ...
    Tile(sc_module_name nm, int id): sc_module(nm) {
		...
		for (int i = 0; i < DIRECTIONS; i++)
		{
			...
			r_req->buffer_full_status_rx[DIRECTIONS+i] (buffer_full_status_ty_local[i]);
			...
			r_req->buffer_full_status_tx[DIRECTIONS+i] (buffer_full_status_ry_local[i]);

			r_req->free_slots[DIRECTIONS+i] (free_slots_local_y[i]);
			r_req->free_slots_neighbor[DIRECTIONS+i] (free_slots_neighbor_local_y[i]);
		}
