...
SC_MODULE(Router)
{
    ...
    sc_in <bool> req_rx[2*DIRECTIONS + 1];	  // The requests associated with the input channels
    sc_out <bool> ack_rx[2*DIRECTIONS + 1];	  // The outgoing ack signals associated with the input channels
    sc_out <TBufferFullStatus> buffer_full_status_rx[2*DIRECTIONS + 1];

    sc_out <Flit> flit_tx[2*DIRECTIONS + 1];   // The output channels
    sc_out <bool> req_tx[2*DIRECTIONS + 1];	  // The requests associated with the output channels
    sc_in <bool> ack_tx[2*DIRECTIONS + 1];	  // The outgoing ack signals associated with the output channels
    ...
    bool current_level_rx[2*DIRECTIONS + 1];	// Current level for Alternating Bit Protocol (ABP)
    bool current_level_tx[2*DIRECTIONS + 1];	// Current level for Alternating Bit Protocol (ABP)
