...
void Hub::txRadioProcessTokenPacket(int channel) {
  ...
  if (current_holder == local_id && current_channel_flag != RELEASE_CHANNEL) {
    if (!init[channel]->buffer_tx.IsEmpty()) {
      ...
      // TODO: check whether it would make sense to use transmission_in_progress
      // to avoid multiple notify()
      LOG << "*** [Ch" << channel << "] Requesting transmission event of flit "
          << flit << endl;
      init[channel]->start_request_event.notify();
    } else {
      if (!transmission_in_progress.at(channel)) {
        LOG << "*** [Ch" << channel
            << "] Buffer_tx empty and no trasmission in progress, releasing "
               "token"
            << endl;
        flag[channel]->write(RELEASE_CHANNEL);
      } else
        LOG << "*** [Ch" << channel
            << "] Buffer_tx empty, but trasmission in progress, holding token"
            << endl;
    }
  }
}
...
1) data received from a radio channel stored (if possible) to a specific
buffer_rx 2) data found on a buffer_rx moved to a buffer_to_tile 3) data found
on a buffer_to_tile moved to signal_tx

From a implementation perspective, they are performed in 3-2-1 order, to
simulate a kind of pipelined sequence
***********************************************************************************/
    ...
    for (int k = 0; k < GlobalParams::n_virtual_channels; k++) {
      ...
      if (!buffer_to_tile[i][vc].IsEmpty()) {
        ...
        if (current_level_tx[i] == ack_tx[i].read() &&
          ...
          LOG << "Flit " << flit << " cannot move from buffer_to_tile[" << i
              << "] [" << vc << "] to signal flit_tx[" << i << "] " << endl;
        }
      } // if buffer not empty
    }
  ...
  // 1) stores routing decision about the incoming flit (e.g., to which output
  // port) 2) Moves the flits removing from antenna buffer_rx
  ...
  for (unsigned int i = 0; i < rxChannels.size(); i++) {
    ...
    if (!(target[channel]->buffer_rx.IsEmpty())) {
      ...
      if (received_flit.flit_type(FLIT_TYPE_HEAD)) {
        ...
        LOG << " Checking reservation availability of output port " << dst_port
            << " by channel " << channel << " for flit " << received_flit
            << endl;

        int rt_status =
            antenna2tile_reservation_table.checkReservation(r, dst_port, 0);

        if (rt_status == RT_AVAILABLE) {
          LOG << "Reserving output port " << dst_port << " by channel "
              << channel << " for flit " << received_flit << endl;
          antenna2tile_reservation_table.reserve(r, dst_port, 0);

          // The number of commucation using the wireless network, accounting
          // also partial wired path
          wireless_communications_counter++;
        } else if (rt_status == RT_ALREADY_SAME) {
          LOG << " RT_ALREADY_SAME reserved direction " << dst_port
              << " for flit " << received_flit << endl;
        } else if (rt_status == RT_OUTVC_BUSY) {
          LOG << " RT_OUTVC_BUSY reservation direction " << dst_port
              << " for flit " << received_flit << endl;
        } else
    ...
    if (reservations.size() != 0) {
      ...
      if (!(target[channel]->buffer_rx.IsEmpty())) {
        ...
        if (!buffer_to_tile[port][vc].IsFull()) {
          ...
          LOG << "*** [Ch" << channel << "] Moving flit  " << received_flit
              << " from buffer_rx to buffer_to_tile[" << port << "][" << vc
              << "]" << endl;
          ...
          if (received_flit.flit_type(FLIT_TYPE_TAIL)) {
            LOG << "Releasing reservation for output port " << port << ", flit "
                << received_flit << endl;
            TReservation r;
            r.input = channel;
            r.vc = vc;
        ...
        // should be ok
        /*
LOG << "WARNING: empty target["<<channel<<"] buffer_rx, but reservation still
present, if correct, remove assertion below " << endl; assert(false);
*/
      }
    }
  }
...
void Hub::tileToAntennaProcess() {
  ...
  for (int j = 0; j < num_ports; j++) {
    ...
    for (int k = 0; k < GlobalParams::n_virtual_channels; k++) {
      ...
      if (!buffer_from_tile[i][vc].IsEmpty()) {
        LOG << "Reservation: buffer_from_tile[" << i << "][" << vc
            << "] not empty " << endl;
        ...
        if (flit.flit_type(FLIT_TYPE_HEAD)) {
          ...
          assert(channel != NOT_VALID &&
                 "hubs are not connected by any channel");

          LOG << "Checking reservation availability of Channel " << channel
              << " by Hub port[" << i << "][" << vc << "] for flit " << flit
              << endl;

          int rt_status =
              tile2antenna_reservation_table.checkReservation(r, channel, 0);

          if (rt_status == RT_AVAILABLE) {
            LOG << "Reservation of channel " << channel << " from Hub port["
                << i << "][" << vc << "] by flit " << flit << endl;
            tile2antenna_reservation_table.reserve(r, channel, 0);
          } else if (rt_status == RT_ALREADY_SAME) {
            LOG << "RT_ALREADY_SAME reserved channel " << channel
                << " for flit " << flit << endl;
          } else if (rt_status == RT_OUTVC_BUSY) {
            LOG << "RT_OUTVC_BUSY reservation for channel " << channel
                << " for flit " << flit << endl;
          } else if (rt_status == RT_ALREADY_OTHER_OUT) {
            LOG << "RT_ALREADY_OTHER_OUT a channel different from " << channel
                << " already reserved by Hub port[" << i << "][" << vc << "]"
                << endl;
          } else
            assert(false); // no meaningful status here
        }
      ...
      if (!buffer_from_tile[i][vc].IsEmpty()) {
        ...
        if (channel != NOT_RESERVED) {
          if (!(init[channel]->buffer_tx.IsFull())) {
            ...
            LOG << "Flit " << flit << " moved from buffer_from_tile[" << i
                << "][" << vc << "]  to buffer_tx[" << channel << "] " << endl;
          } else {
          ...
          LOG << "Forwarding: No channel reserved for input port [" << i << "]["
              << vc << "], flit " << flit << endl;
        }
      }
  ...
  for (int i = 0; i < num_ports; i++) {
    ...
    if (req_rx[i]->read() == 1 - current_level_rx[i]) {
      ...
      LOG << "Reading " << received_flit << " from signal flit_rx[" << i << "]"
          << endl;
...
*/
      ...
      if (!buffer_from_tile[i][vc].IsFull()) {
        LOG << "Storing " << received_flit << " on buffer_from_tile[" << i
            << "][" << vc << "]" << endl;
        ...
        LOG << "Buffer Full: Cannot store " << received_flit
            << " on buffer_from_tile[" << i << "][" << vc << "]" << endl;
  ...
  if (GlobalParams::channel_selection == CHSEL_RANDOM)
    ...
    for (vector<int>::size_type i = 0; i < intersection.size(); i++) {
      ...
      if (!transmission_in_progress.at(intersection[k])) {
        cout << "Found free channel " << intersection[k] << " on (src,dest) ("
             << src_hub << "," << dst_hub << ") " << endl;
