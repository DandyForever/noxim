...
unsigned long GlobalStats::getLocalDirectionsTotal(
    vector<vector<vector<unsigned long>>> &stat_mtx, Coord coord) {
    ...
    for (int x = 0; x < GlobalParams::mesh_dim_x; x++)
      for (int y_ = 0; y_ < GlobalParams::mesh_dim_y; y_++)
        for (int x_ = 0; x_ < GlobalParams::mesh_dim_x; x_++)
          if (dir == 'x')
            mtx[y][x] += noc->t[x_][y_]->r->stats.getFlitsSrcCount(
                y * GlobalParams::mesh_dim_x + x);
          else
            mtx[y][x] += noc->t[x_][y_]->r_req->stats.getFlitsSrcCount(
                y * GlobalParams::mesh_dim_x + x);
...
void GlobalStats::showStats(std::ostream &out, bool detailed) {
  ...
  if (detailed) {
    ...
    file_name << GlobalParams::log_file_name << "_mesh_"
    ...
    std::ofstream f_latencies_burst(file_name.str() + "_latency_burst.log",
                                    std::ofstream::out);
    std::ofstream f_latencies_creation_burst(file_name.str() + "_latency_creation_burst.log",
                                    std::ofstream::out);
    ...
    for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
      for (int x = 0; x < GlobalParams::mesh_dim_x; x++)
        noc->t[x][y]->r_req->stats.showStats(y * GlobalParams::mesh_dim_x + x,
                                             out, true);
    out << "];" << endl;
    //-----------------------------------------------------------------------
    // End Router stats
      ...
      for (int d = 0; d < DIRECTIONS + GlobalParams::mem_ports; d++) {
        ...
        for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) {
          ...
          for (int y = 0; y < GlobalParams::mesh_dim_y; y++) {
            for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
              if (vc == 0)
                out << "(" << noc->t[x][y]->r->buffer_out[d][0].Size() << ", "
                    << noc->t[x][y]->r_req->buffer_out[d][1].Size() << ")"
                    << " ";
            }
            out << endl;
          }
      ...
      for (int d = 0; d < DIRECTIONS + GlobalParams::mem_ports; d++) {
        ...
        for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) {
          ...
          for (int y = 0; y < GlobalParams::mesh_dim_y; y++) {
            for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
              if (vc == 0)
                out << "(" << noc->t[x][y]->r->buffer[d][0].Size() << ", "
                    << noc->t[x][y]->r_req->buffer[d][1].Size() << ")"
                    << " ";
            }
            out << endl;
          }
      ...
      for (int d = 0; d < DIRECTIONS + GlobalParams::mem_ports; d++) {
        ...
        for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) {
          ...
          for (int y = 0; y < GlobalParams::mesh_dim_y; y++) {
            for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
              if (vc == 0)
                out << "(" << noc->t[x][y]->r_req->buffer_out[d][0].Size()
                    << ", " << noc->t[x][y]->r_req->buffer_out[d][1].Size()
                    << ")"
                    << " ";
            }
      ...
      for (int d = 0; d < DIRECTIONS + GlobalParams::mem_ports; d++) {
        ...
        for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) {
          ...
          for (int y = 0; y < GlobalParams::mesh_dim_y; y++) {
            for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
              if (vc == 0)
                out << "(" << noc->t[x][y]->r_req->buffer[d][0].Size() << ", "
                    << noc->t[x][y]->r_req->buffer[d][1].Size() << ")"
                    << " ";
            }
            out << endl;
    ...
    //-----------------------------------------------------------------------
    // End PE flit request latency burst
    //-----------------------------------------------------------------------

    //-----------------------------------------------------------------------
    // Start PE flit request latency from creation burst
    //-----------------------------------------------------------------------
    for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
      for (int y = 0; y < GlobalParams::mesh_dim_y; y++) {
        for (auto it =
                 noc->t[x][y]->pe[0]->traffic_burst_flit_latency_from_creation_x.begin();
             it != noc->t[x][y]->pe[0]->traffic_burst_flit_latency_from_creation_x.end();
             it++) {
          Coord dst_coord = id2Coord(it->second.dst_id);
          int latency = it->second.latency;
          if (it->second.is_back)
        ...
        for (auto it =
                 noc->t[x][y]->pe[0]->traffic_burst_flit_latency_from_creation_y.begin();
             it != noc->t[x][y]->pe[0]->traffic_burst_flit_latency_from_creation_y.end();
             it++) {
          Coord dst_coord = id2Coord(it->second.dst_id);
          int latency = it->second.latency;
          if (it->second.is_back)
    ...
    f_latencies_creation_burst.close();
    //-----------------------------------------------------------------------
    // End PE flit request latency from creation burst
    //-----------------------------------------------------------------------
...
void GlobalStats::showPowerBreakDown(std::ostream &out) {
  ...
  } else // other delta topologies
  {
    for (int y = 0; y < GlobalParams::n_delta_tiles; y++) {
      updatePowerBreakDown(power_dynamic,
                           noc->core[y]->r->power.getDynamicPowerBreakDown());
      updatePowerBreakDown(power_static,
                           noc->core[y]->r->power.getStaticPowerBreakDown());
    }
  }
