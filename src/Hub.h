...
SC_MODULE(Hub) {
  ...
  int *start_from_vc;  // VC from which to start the reservation cycle for the
                       // specific port
...
#define STATIC_MAX_CHANNELS 100
    ...
    for (int i = 0; i < num_ports; i++) {
      for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) {
        buffer_from_tile[i][vc].SetMaxBufferSize(
            GlobalParams::hub_configuration[local_id].fromTileBufferSize);
        buffer_to_tile[i][vc].SetMaxBufferSize(
            GlobalParams::hub_configuration[local_id].toTileBufferSize);
        buffer_from_tile[i][vc].setLabel(string(name()) + "->bft[" +
                                         i_to_string(i) + "][" +
                                         i_to_string(vc) + "]");
        buffer_to_tile[i][vc].setLabel(string(name()) + "->btt[" +
                                       i_to_string(i) + "][" + i_to_string(vc) +
                                       "]");
      }
      start_from_vc[i] = 0;
    }
    ...
    for (unsigned int i = 0; i < txChannels.size(); i++) {
      ...
      init[ch]->buffer_tx.SetMaxBufferSize(
      ...
      token_ring->attachHub(ch, local_id, current_token_holder[ch],
                            current_token_expiration[ch], flag[ch]);
    ...
    for (unsigned int i = 0; i < rxChannels.size(); i++) {
      ...
      target[rxChannels[i]]->buffer_rx.setLabel(string(name()) + "->abrx[" +
                                                i_to_string(i) + "]");
    }
