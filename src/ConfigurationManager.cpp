...
static std::vector<Rect> parse_rect_list_or_single(const YAML::Node &parent) {
  std::vector<Rect> rects;
  auto sa = parent["slave_array"];
  if (sa && sa.IsDefined()) {
    rects.push_back(parse_rect(sa));
  }
  auto sas = parent["slave_areas"];
  if (sas && sas.IsDefined()) {
    if (!sas.IsSequence()) {
      std::cerr << "slave_areas must be a sequence\n";
      std::exit(1);
    }
    for (const auto &item : sas)
      rects.push_back(parse_rect(item));
  }
  return rects;
}
...
void loadConfiguration() {
  ...
  if (GlobalParams::topology == TOPOLOGY_MESH &&
      (GlobalParams::mesh_dim_x < 3 || GlobalParams::mesh_dim_y < 3)) {
    std::cerr
        << "mesh_dim must be at least 3x3 to have interior memory nodes\n";
    std::exit(1);
  }

  // 1) global slave_area
  GlobalParams::global_slave_areas = parse_rect_list_or_single(config);
  if (GlobalParams::global_slave_areas.empty()) {
    GlobalParams::global_slave_areas.push_back(Rect{
  ...
  if (config["master_slave_rules"]) {
    for (const auto &rule : config["master_slave_rules"]) {
      if (!rule["select"]) {
        cerr << "each rule must have select";
        exit(1);
      }
      auto rects = parse_rect_list_or_single(rule);
      if (rects.empty()) {
        std::cerr << "rule must have slave_array or slave_areas\n";
        std::exit(1);
      }
