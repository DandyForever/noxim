...
*/
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int pe_id = 0; pe_id < DIRECTIONS; pe_id++) {
      ...
      core[i]->pe[pe_id]->local_direction_id = DIRECTIONS + pe_id;
      // Check for traffic table availability
      if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED) {
      ...
      for (int pe_id = 0; pe_id < DIRECTIONS; pe_id++) {
        ...
        if (GlobalParams::interliving_direction) {
          if (t[i][j]->pe[pe_id]->local_id % GlobalParams::mesh_dim_x == 0)
            t[i][j]->pe[pe_id]->interliving_prev_dst =
                t[i][j]->pe[pe_id]->local_id + GlobalParams::mesh_dim_x - 1;
          else
            t[i][j]->pe[pe_id]->interliving_prev_dst =
                t[i][j]->pe[pe_id]->local_id - GlobalParams::mesh_dim_x + 1;
        }
        t[i][j]->pe[pe_id]->interliving_prev_reps =
            GlobalParams::interliving_reps;
