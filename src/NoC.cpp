...
void NoC::buildButterfly() {
  ...
  for (int j = 0; j < dimY; j++) {
    for (int i = 0; i < dimX; i++) {
      ...
      sprintf(tile_name, "Switch[%d][%d]_(#%d)", i, j,
              tile_id); // cout<<"tile_name=" <<tile_name<< " i=" <<i << " j="
                        // << j<< " tile_id "<< tile_id<< endl;
      t[i][j] = new Tile(tile_name, tile_id);

      // cout << "switch  " << i <<  " " << j << "   has an Id = " << tile_id <<
      // endl;
      ...
      for (int pe_id = 0; pe_id < LOCAL_DIRECTIONS; pe_id++) {
        t[i][j]->pe[pe_id]->local_id = tile_id;
        t[i][j]->pe[pe_id]->local_direction_id = pe_id_to_local_direction(pe_id);
        t[i][j]->pe[pe_id]->traffic_table =
            &gttable; // Needed to choose destination
        t[i][j]->pe[pe_id]->never_transmit = true;
      }
      ...
      if (it != GlobalParams::hub_for_tile.end()) {
        ...
        hub[hub_id]->buffer_full_status_rx[port](
            buffer_full_status[i][j].from_hub);

        hub[hub_id]->flit_tx[port](flit[i][j].from_hub);
        hub[hub_id]->req_tx[port](req[i][j].from_hub);
        hub[hub_id]->ack_tx[port](ack[i][j].to_hub);
        hub[hub_id]->buffer_full_status_tx[port](
            buffer_full_status[i][j].to_hub);
      }
    }
  ...
  // declaration of dummy signals used for the useless Tx and Rx connection in
  // Butterfly
  ...
  sc_signal<TBufferFullStatus> *tbufferfullstatus_dummy_signal =
      new sc_signal<TBufferFullStatus>;
    ...
    for (int j = 0; j < sw; j++) // sw
    {
      ...
      if (d == 0) {
        if (i % 2 == 0) // stage even
        {
          ...
          t[i - 1][j]->buffer_full_status_rx[d](
              *tbufferfullstatus_dummy_signal);
          ...
          t[i - 1][m]->buffer_full_status_rx[d](
              *tbufferfullstatus_dummy_signal);
        }
          ...
          t[i - 1][j]->buffer_full_status_tx[d](
              buffer_full_status[i - 1][j].north);
          // tx signals not required for delta topologies
          t[i - 1][j]->flit_rx[d](*flit_dummy_signal);
          t[i - 1][j]->req_rx[d](*bool_dummy_signal);
          t[i - 1][j]->ack_rx[d](*bool_dummy_signal);
          ...
          t[i - 1][m]->buffer_full_status_tx[d](
              buffer_full_status[i - 1][m].north);
          // tx signals not required for delta topologies
          t[i - 1][m]->flit_rx[d](*flit_dummy_signal);
          t[i - 1][m]->req_rx[d](*bool_dummy_signal);
          t[i - 1][m]->ack_rx[d](*bool_dummy_signal);
...
*/
  ...
  // signals instantiation for connecting Core2Hub (just to test wioreless in
  // Butterfly)
  ...
  for (int i = 0; i < n; i++) {
    ...
    sprintf(core_name, "Core_(#%d)",
            core_id); // cout<< "core_id = "<< core_id << endl;
    ...
    // Tell to the PE its coordinates
    for (int pe_id = 0; pe_id < LOCAL_DIRECTIONS; pe_id++) {
      core[i]->pe[pe_id]->local_id = core_id;
      core[i]->pe[pe_id]->local_direction_id = pe_id_to_local_direction(pe_id);
      // Check for traffic table availability
      if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED) {
        core[i]->pe[pe_id]->traffic_table =
            &gttable; // Needed to choose destination
        core[i]->pe[pe_id]->never_transmit =
            (gttable.occurrencesAsSource(core[i]->pe[pe_id]->local_id) == 0);
      } else
        core[i]->pe[pe_id]->never_transmit = false;
    }
    ...
    // TODO: Review port index. Connect each Hub to all its Channels // connect
    // Hub2Core
    // for (map<int, int>::iterator it1 = GlobalParams::hub_for_tile.begin();
    // it1 != GlobalParams::hub_for_tile.end(); it1++ ) LOG<<"it1 first "<<
    // it1->first<< "second"<< it1->second<<endl;
    ...
    if (it != GlobalParams::hub_for_tile.end()) {
      ...
      // port will be connected
      int port = hub_connected_ports[hub_id]++;
      // LOG<<"I am hub "<<hub_id<<" connecting to core "<<core_id<<"using port
      // "<<port<<endl;
      ...
      hub[hub_id]->buffer_full_status_rx[port](
          buffer_full_status_from_hub[core_id]);
      ...
      hub[hub_id]->buffer_full_status_tx[port](
          buffer_full_status_to_hub[core_id]);
    }
  }
...
*/
  ...
  for (int i = 0; i < sw; i++) {
    ...
    core[(i * 2) + 1]->buffer_full_status_rx[0](
        *tbufferfullstatus_dummy_signal);
  }
  ...
  for (int i = 0; i < sw; i++) {
    ...
    t[stg - 1][i]->buffer_full_status_tx[0](
        buffer_full_status[stg - 1][i].north);
    // rx is not required in delta topologies
    ...
    t[stg - 1][i]->buffer_full_status_tx[1](
        buffer_full_status[stg - 1][i].east);
    // rx is not required in delta topologies
...
void NoC::buildBaseline() {
  ...
  // NOTE: the only difference between Baseline and Butterfly mapping
  // architecture is the first stage connections First Stage Mapping(Stage 1)
  for (int j = 0; j < sw; j++) // sw
  {
    ...
    if (d == 0) {
      ...
      t[1][j]->buffer_full_status_rx[2](
          buffer_full_status[0][(2 * j) + 1].north);
      // tx signals not required for delta topologies
      ...
      t[1][j]->buffer_full_status_rx[3](
          buffer_full_status[0][2 * (j - r)].east);
      // tx signals not required for delta topologies
      t[1][j]->flit_tx[3](*flit_dummy_signal);
      t[1][j]->req_tx[3](*bool_dummy_signal);
      t[1][j]->ack_tx[3](*bool_dummy_signal);
      ...
      t[0][2 * (j - r)]->buffer_full_status_tx[d](
          buffer_full_status[0][2 * (j - r)].east);
      // rx signals not required for delta topologies
      t[0][2 * (j - r)]->flit_rx[d](*flit_dummy_signal);
      t[0][2 * (j - r)]->req_rx[d](*bool_dummy_signal);
      t[0][2 * (j - r)]->ack_rx[d](*bool_dummy_signal);
      t[0][2 * (j - r)]->buffer_full_status_rx[d](
          *tbufferfullstatus_dummy_signal);

      //*****Direction 2 *****

      // The switch sw[1,j] connected to direction d of sw[0,2*(j-r)+1 ] --> dir
      // 2
      ...
      t[1][j]->buffer_full_status_rx[2](
          buffer_full_status[0][(2 * (j - r)) + 1].east);
      // tx signals not required for delta topologies
      t[1][j]->flit_tx[2](*flit_dummy_signal);
      t[1][j]->req_tx[2](*bool_dummy_signal);
      t[1][j]->ack_tx[2](*bool_dummy_signal);
  ...
  int n = GlobalParams::n_delta_tiles; // n: nombre of Cores = tiles with 2
                                       // directions(0 & 1)
...
*/
          ...
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_tx[0](
              buffer_full_status[i][j].south);
          // tx signals not required for delta topologies
          ...
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_rx[0](
              *tbufferfullstatus_dummy_signal);
        }
          ...
          t[i - 1][j / 2]->buffer_full_status_tx[d](
              buffer_full_status[i][j].west);
          // tx signals not required for delta topologies
          t[i - 1][j / 2]->flit_rx[1](*flit_dummy_signal);
          t[i - 1][j / 2]->req_rx[1](*bool_dummy_signal);
          t[i - 1][j / 2]->ack_rx[1](*bool_dummy_signal);
          t[i - 1][j / 2]->buffer_full_status_rx[1](
              *tbufferfullstatus_dummy_signal);

          //*** Direction 2 ****
          // sw(i,j) get connected to the direction 1 of sw(i-1,(n/4)+(j/2)) -->
          // dir2
          t[i][j]->flit_rx[2](flit[i][j].south);
          t[i][j]->req_rx[2](req[i][j].south);
          t[i][j]->ack_rx[2](ack[i][j].south);
          ...
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_tx[1](
              buffer_full_status[i][j].south);
          // tx signals not required for delta topologies
          ...
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_rx[1](
              *tbufferfullstatus_dummy_signal);
        }
      ...
      } else // stage odd
      {
        if (j % 2 == 0) {
          ...
          t[i][j]->buffer_full_status_rx[3](
              buffer_full_status[i - 1][j / 2].north);
          // tx signals not required for delta topologies
          t[i][j]->flit_tx[3](*flit_dummy_signal);
          t[i][j]->req_tx[3](*bool_dummy_signal);
          t[i][j]->ack_tx[3](*bool_dummy_signal);
          ...
          //*****Direction 2 *****
          // sw(i,j) get connected to the direction 0 of sw(i-1,(n/4)+(j/2))-->
          // dir2

          t[i][j]->flit_rx[2](flit[i - 1][(n / 4) + (j / 2)].north);
          t[i][j]->req_rx[2](req[i - 1][(n / 4) + (j / 2)].north);
          t[i][j]->ack_rx[2](ack[i - 1][(n / 4) + (j / 2)].north);
          t[i][j]->buffer_full_status_rx[2](
              buffer_full_status[i - 1][(n / 4) + (j / 2)].north);
          // tx signals not required for delta topologies
          t[i][j]->flit_tx[2](*flit_dummy_signal);
          t[i][j]->req_tx[2](*bool_dummy_signal);
          t[i][j]->ack_tx[2](*bool_dummy_signal);
          ...
          t[i - 1][(n / 4) + (j / 2)]->flit_tx[0](
              flit[i - 1][(n / 4) + (j / 2)].north);
          t[i - 1][(n / 4) + (j / 2)]->req_tx[0](
              req[i - 1][(n / 4) + (j / 2)].north);
          t[i - 1][(n / 4) + (j / 2)]->ack_tx[0](
              ack[i - 1][(n / 4) + (j / 2)].north);
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_tx[0](
              buffer_full_status[i - 1][(n / 4) + (j / 2)].north);
          // rx signals not required for delta topologies
          t[i - 1][(n / 4) + (j / 2)]->flit_rx[0](*flit_dummy_signal);
          ...
          t[i][j]->buffer_full_status_rx[3](
              buffer_full_status[i - 1][j / 2].east);
          // tx signals not required for delta topologies
          t[i][j]->flit_tx[3](*flit_dummy_signal);
          t[i][j]->req_tx[3](*bool_dummy_signal);
          t[i][j]->ack_tx[3](*bool_dummy_signal);
          ...
          t[i - 1][j / 2]->buffer_full_status_tx[1](
              buffer_full_status[i - 1][j / 2].east);
          // rx signals not required for delta topologies
          t[i - 1][j / 2]->flit_rx[1](*flit_dummy_signal);
          t[i - 1][j / 2]->req_rx[1](*bool_dummy_signal);
          ...
          t[i - 1][(n / 4) + (j / 2)]->flit_tx[1](
              flit[i - 1][(n / 4) + (j / 2)].east);
          t[i - 1][(n / 4) + (j / 2)]->req_tx[1](
              req[i - 1][(n / 4) + (j / 2)].east);
          t[i - 1][(n / 4) + (j / 2)]->ack_tx[1](
              ack[i - 1][(n / 4) + (j / 2)].east);
          t[i - 1][(n / 4) + (j / 2)]->buffer_full_status_tx[1](
              buffer_full_status[i - 1][(n / 4) + (j / 2)].east);
          // rx signals not required for delta topologies
          t[i - 1][(n / 4) + (j / 2)]->flit_rx[1](*flit_dummy_signal);
  ...
  // int n = GlobalParams::n_delta_tiles; //n: nombre of Cores = tiles with 2
  // directions(0 & 1) Dimensions of the delta topologies Cores : dimX=1 &
  // dimY=n instantiation of the Cores (we have only one row)
...
void NoC::buildMesh() {
    ...
    for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
      ...
      for (int pe_id = 0; pe_id < LOCAL_DIRECTIONS; pe_id++) {
        ...
        if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED) {
          t[i][j]->pe[pe_id]->traffic_table =
              &gttable; // Needed to choose destination
          t[i][j]->pe[pe_id]->never_transmit =
              (gttable.occurrencesAsSource(t[i][j]->pe[pe_id]->local_id) == 0);
        } else
          t[i][j]->pe[pe_id]->never_transmit = false;
      }
      ...
      t[i][j]->buffer_full_status_rx[DIRECTION_NORTH](
          buffer_full_status[i][j].north);
      ...
      t[i][j]->buffer_full_status_rx[DIRECTION_EAST](
          buffer_full_status[i + 1][j].east);

      t[i][j]->req_rx[DIRECTION_EAST_EXTRA](req[i + 1][j].west_extra);
      t[i][j]->flit_rx[DIRECTION_EAST_EXTRA](flit[i + 1][j].west_extra);
      t[i][j]->ack_rx[DIRECTION_EAST_EXTRA](ack[i + 1][j].east_extra);
      t[i][j]->buffer_full_status_rx[DIRECTION_EAST_EXTRA](
          buffer_full_status[i + 1][j].east_extra);
      ...
      t[i][j]->buffer_full_status_rx[DIRECTION_SOUTH](
          buffer_full_status[i][j + 1].south);
      ...
      t[i][j]->buffer_full_status_rx[DIRECTION_WEST](
          buffer_full_status[i][j].west);

      t[i][j]->req_rx[DIRECTION_WEST_EXTRA](req[i][j].east_extra);
      t[i][j]->flit_rx[DIRECTION_WEST_EXTRA](flit[i][j].east_extra);
      t[i][j]->ack_rx[DIRECTION_WEST_EXTRA](ack[i][j].west_extra);
      t[i][j]->buffer_full_status_rx[DIRECTION_WEST_EXTRA](
          buffer_full_status[i][j].west_extra);
      ...
      t[i][j]->buffer_full_status_tx[DIRECTION_NORTH](
          buffer_full_status[i][j].south);
      ...
      t[i][j]->buffer_full_status_tx[DIRECTION_EAST](
          buffer_full_status[i + 1][j].west);

      t[i][j]->req_tx[DIRECTION_EAST_EXTRA](req[i + 1][j].east_extra);
      t[i][j]->flit_tx[DIRECTION_EAST_EXTRA](flit[i + 1][j].east_extra);
      t[i][j]->ack_tx[DIRECTION_EAST_EXTRA](ack[i + 1][j].west_extra);
      t[i][j]->buffer_full_status_tx[DIRECTION_EAST_EXTRA](
          buffer_full_status[i + 1][j].west_extra);
      ...
      t[i][j]->buffer_full_status_tx[DIRECTION_SOUTH](
          buffer_full_status[i][j + 1].north);
      ...
      t[i][j]->buffer_full_status_tx[DIRECTION_WEST](
          buffer_full_status[i][j].east);

      t[i][j]->req_tx[DIRECTION_WEST_EXTRA](req[i][j].west_extra);
      t[i][j]->flit_tx[DIRECTION_WEST_EXTRA](flit[i][j].west_extra);
      t[i][j]->ack_tx[DIRECTION_WEST_EXTRA](ack[i][j].east_extra);
      t[i][j]->buffer_full_status_tx[DIRECTION_WEST_EXTRA](
          buffer_full_status[i][j].east_extra);
      //-------------------------------------------------------------------------
      ...
      t[i][j]->buffer_full_status_ry[DIRECTION_NORTH](
          buffer_full_status_y[i][j].north);
      ...
      t[i][j]->buffer_full_status_ry[DIRECTION_EAST](
          buffer_full_status_y[i + 1][j].east);

      t[i][j]->req_ry[DIRECTION_EAST_EXTRA](req_y[i + 1][j].west_extra);
      t[i][j]->flit_ry[DIRECTION_EAST_EXTRA](flit_y[i + 1][j].west_extra);
      t[i][j]->ack_ry[DIRECTION_EAST_EXTRA](ack_y[i + 1][j].east_extra);
      t[i][j]->buffer_full_status_ry[DIRECTION_EAST_EXTRA](
          buffer_full_status_y[i + 1][j].east_extra);
      ...
      t[i][j]->buffer_full_status_ry[DIRECTION_SOUTH](
          buffer_full_status_y[i][j + 1].south);
      ...
      t[i][j]->buffer_full_status_ry[DIRECTION_WEST](
          buffer_full_status_y[i][j].west);

      t[i][j]->req_ry[DIRECTION_WEST_EXTRA](req_y[i][j].east_extra);
      t[i][j]->flit_ry[DIRECTION_WEST_EXTRA](flit_y[i][j].east_extra);
      t[i][j]->ack_ry[DIRECTION_WEST_EXTRA](ack_y[i][j].west_extra);
      t[i][j]->buffer_full_status_ry[DIRECTION_WEST_EXTRA](
          buffer_full_status_y[i][j].west_extra);
      ...
      t[i][j]->buffer_full_status_ty[DIRECTION_NORTH](
          buffer_full_status_y[i][j].south);
      ...
      t[i][j]->buffer_full_status_ty[DIRECTION_EAST](
          buffer_full_status_y[i + 1][j].west);

      t[i][j]->req_ty[DIRECTION_EAST_EXTRA](req_y[i + 1][j].east_extra);
      t[i][j]->flit_ty[DIRECTION_EAST_EXTRA](flit_y[i + 1][j].east_extra);
      t[i][j]->ack_ty[DIRECTION_EAST_EXTRA](ack_y[i + 1][j].west_extra);
      t[i][j]->buffer_full_status_ty[DIRECTION_EAST_EXTRA](
          buffer_full_status_y[i + 1][j].west_extra);
      ...
      t[i][j]->buffer_full_status_ty[DIRECTION_SOUTH](
          buffer_full_status_y[i][j + 1].north);
      ...
      t[i][j]->buffer_full_status_ty[DIRECTION_WEST](
          buffer_full_status_y[i][j].east);

      t[i][j]->req_ty[DIRECTION_WEST_EXTRA](req_y[i][j].west_extra);
      t[i][j]->flit_ty[DIRECTION_WEST_EXTRA](flit_y[i][j].west_extra);
      t[i][j]->ack_ty[DIRECTION_WEST_EXTRA](ack_y[i][j].east_extra);
      t[i][j]->buffer_full_status_ty[DIRECTION_WEST_EXTRA](
          buffer_full_status_y[i][j].east_extra);
      //-------------------------------------------------------------------------
      ...
      t[i][j]->free_slots[DIRECTION_EAST_EXTRA](
          free_slots[i + 1][j].east_extra);
      t[i][j]->free_slots[DIRECTION_SOUTH](free_slots[i][j + 1].south);
      t[i][j]->free_slots[DIRECTION_WEST](free_slots[i][j].west);
      t[i][j]->free_slots[DIRECTION_WEST_EXTRA](free_slots[i][j].west_extra);
      ...
      t[i][j]->free_slots_neighbor[DIRECTION_EAST_EXTRA](
          free_slots[i + 1][j].west_extra);
  ...
  for (int j = 0; j <= GlobalParams::mesh_dim_y; j++) {
    ...
    //-------------------------------------------------------------------------
    req[0][j].east = 0;
    req[0][j].east_extra = 0;
    ack[0][j].west = 0;
    ack[0][j].west_extra = 0;
    req[GlobalParams::mesh_dim_x][j].west = 0;
    req[GlobalParams::mesh_dim_x][j].west_extra = 0;
    ack[GlobalParams::mesh_dim_x][j].east = 0;
    ack[GlobalParams::mesh_dim_x][j].east_extra = 0;

    free_slots[0][j].east.write(NOT_VALID);
    free_slots[0][j].east_extra.write(NOT_VALID);
    free_slots[GlobalParams::mesh_dim_x][j].west.write(NOT_VALID);
    free_slots[GlobalParams::mesh_dim_x][j].west_extra.write(NOT_VALID);

    nop_data[0][j].east.write(tmp_NoP);
    nop_data[0][j].east_extra.write(tmp_NoP);
    nop_data[GlobalParams::mesh_dim_x][j].west.write(tmp_NoP);
    nop_data[GlobalParams::mesh_dim_x][j].west_extra.write(tmp_NoP);
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    req_y[0][j].east = 0;
    req_y[0][j].east_extra = 0;
    ack_y[0][j].west = 0;
    ack_y[0][j].west_extra = 0;
    req_y[GlobalParams::mesh_dim_x][j].west = 0;
    req_y[GlobalParams::mesh_dim_x][j].west_extra = 0;
    ack_y[GlobalParams::mesh_dim_x][j].east = 0;
    ack_y[GlobalParams::mesh_dim_x][j].east_extra = 0;

    free_slots_y[0][j].east.write(NOT_VALID);
    free_slots_y[0][j].east_extra.write(NOT_VALID);
    free_slots_y[GlobalParams::mesh_dim_x][j].west.write(NOT_VALID);
    free_slots_y[GlobalParams::mesh_dim_x][j].west_extra.write(NOT_VALID);

    nop_data_y[0][j].east.write(tmp_NoP);
    nop_data_y[0][j].east_extra.write(tmp_NoP);
    nop_data_y[GlobalParams::mesh_dim_x][j].west.write(tmp_NoP);
    nop_data_y[GlobalParams::mesh_dim_x][j].west_extra.write(tmp_NoP);
    //-------------------------------------------------------------------------
  }
}
  ...
  for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
    for (int s = 0; s < 3; s++) {
      for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
        if (s == 0)
          std::printf("|  %d  ",
                      t[i][j]->r->buffer[s][0].getCurrentFreeSlots());
        else if (s == 1)
          std::printf("|%d   %d",
                      t[i][j]->r->buffer[s][0].getCurrentFreeSlots(),
                      t[i][j]->r->buffer[3][0].getCurrentFreeSlots());
        else
          std::printf("|__%d__",
                      t[i][j]->r->buffer[2][0].getCurrentFreeSlots());
      }
      cout << endl;
    }
  }
